  ...
  return (
    <>
      ...
      <header className="bg-[#2A2A2A] border-b border-[#3A3A3A]" role="banner">
        <nav className="container mx-auto px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center" role="navigation" aria-label="Navegação principal">
          {/* Logo */}
          <button 
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 md:space-x-6 flex-shrink-0">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-4 w-4 md:h-5 md:w-5 flex items-center justify-center font-bold" aria-hidden="true">
            ...
            {isAuthenticated && user ? (
              <>
                ...
                >
                  <Avatar name={user.name} size={40} className="border-2 border-accent-purple" />
                  <span className="hidden md:block text-main-text text-sm">{user.name}</span>
                </button>
